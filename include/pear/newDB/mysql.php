...
class DB_mysql extends DB_common
{
    ...
    function connect($dsninfo, $persistent = false)
    {
        ...
        // @@@ CK testing
       // $db = @mysql_connect('db2.library.uq.edu.au', 'eventum', 'p55eventum');
       // mysql_select_db('dev_eventum', $db);  
       // $this->connection = $db;
        return DB_OK;
    }
...
//        $db = @mysql_connect('db2.library.uq.edu.au', 'eventum', 'p55eventum');
//        mysql_select_db('dev_eventum', $db);  
  //      $q2 = "select * from dev_eventum.eventum_issue";
$q2 = "SELECT
                    fld_id,
                    fld_title,
                    fld_description,
                 ...
                 FROM
                    dev_eventum.eventum_custom_field,
                    dev_eventum.eventum_project_custom_field
                 WHERE
                    pcf_fld_id=fld_id AND
                    pcf_prj_id=4 AND
fld_report_form=1 ORDER BY fld_id ASC";
...
$q3 = "SELECT
                    ...
                    prj_title
                 FROM
                    dev_eventum.eventum_project,
                    dev_eventum.eventum_project_user
                 WHERE
...
//$q2 = $this->modifyQuery($q2);
        //$restemp = @mysql_query($q2, $db);
//        echo "THIS IS THE QUERY -> ".$query;
//        $restemp = @mysql_query($query, $db);
        $restemp = @mysql_query($query, $this->connection);
        $temp = array();
        $temp = @mysql_fetch_array($restemp, MYSQL_NUM);
//        $tmp = mysql_fetch_array($restemp, MYSQL_ASSOC);
//        echo "MUAHAHHAAH"; print_r($tmp);
//        mysql_close($db);
        //$restemp = null;
        //$tmp = null;
        print_r($temp);
//        print_r
//        print_r(mysql_fetch_array($result)); echo "JUST DID SIMPLE MYSQL QUERY";
             /* $recordset = array();
              for ($i = 0; $i < mysql_num_rows($result; $i++)
              {
                $recordset[$i]=mysql_fetch_assoc($result);
              }*/
              //print_r($recordset);
