...
class Workflow
{
	...
	function remove()
	{
		...
		$stmt = "DELETE FROM
                 ...
                 WHERE
                    wfl_id IN (".Misc::arrayToSQLBindStr($_POST["items"]).")";
		try {
	...
	function associateRole($aro_id, $wfl_id)
	{
		...
		$stmt = "INSERT INTO
                 ...
                 (
                    ...
                    ".$db->quote($wfl_id, 'INTEGER').",
                    ".$db->quote($aro_id, 'INTEGER')."
                 )";
		try {
			$db->query($stmt);
		}
	...
	function getTitle($wfl_id)
	{
		...
		$stmt = "SELECT
                 ...
                 FROM
                    " . APP_TABLE_PREFIX . "workflow
                 WHERE
                    wfl_id=".$db->quote($wfl_id, 'INTEGER');
		try {
	 ...
	 */
	function start($wft_id, $pid, $xdis_id, $href='', $dsID='', $pids=array(), $custom_view_pid='')
	{
		$wfstatus = new WorkflowStatus($pid, $wft_id, $xdis_id, '', $dsID, $pids, $custom_view_pid);
		$wfstatus->href=$href;
		$wfstatus->run();
	}
	...
	function importWorkflow($xworkflow, $behaviour_ids_map, &$feedback)
	{
		...
		$list = Workflow::getList($where="WHERE wfl_title=".$db->quote($title));
		if (!empty($list)) {
			$overwrite = true;
