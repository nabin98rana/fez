...
class WorkflowStatus {
	...
	function clearSession()
	{
		...
		$stmt = "DELETE FROM ".$dbtp."workflow_sessions " .
                "WHERE wfses_id=".$db->quote($id, 'INTEGER')." AND wfses_usr_id=".$db->quote($usr_id, 'INTEGER');
		try {
			$db->query($stmt);
		}
		catch(Exception $ex) {
...
class WorkflowStatusStatic
{
	...
	function getSession($id = null, $usr_id = null, $wfs_id = null)
	{
		...
		$stmt = "SELECT wfses_object FROM ".$dbtp."workflow_sessions " .
                "WHERE wfses_usr_id=".$db->quote($usr_id, 'INTEGER')."  AND wfses_id=".$db->quote($id, 'INTEGER');
		try {
			$log->debug($stmt);
		...
		if (!is_object($obj) || get_class($obj) != 'WorkflowStatus' ) {
			$log->err("Workflow object is corrupt. get_class: ".get_class($obj)."print_r: ".print_r($obj,true),__FILE__,__LINE__);
			return null;
		}
