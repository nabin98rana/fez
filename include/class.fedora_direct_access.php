...
function fetchAllFedoraPIDs($terms = "", $object_state = 'A') {
        ...
        $result = $this->dbh->getAll("SELECT dopid AS pid, label AS title, objectstate FROM doregistry WHERE (dopid LIKE '%" . $terms . "%' OR label LIKE '%" . $terms . "%') AND objectState = 'A'", DB_FETCHMODE_ASSOC);
