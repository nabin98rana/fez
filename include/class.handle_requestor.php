...
class HandleRequestor {
	...
	$res = $GLOBALS["db_api"]->dbh->getAll($stmt, DB_FETCHMODE_ASSOC);
	    ...
	    Error_Handler::logError(array("Handle name space not defined: "), __FILE__, __LINE__);
	} else if (PEAR::isError($res)) {
	    Error_Handler::logError(array("Handle search key not setup properly: ", $res->getMessage(), $res->getDebugInfo()), __FILE__, __LINE__);
	} else {
	...
	Error_Handler::logError("JAHDL Setup Error: is the path to the JAHDL root directory setup properly?\n Return status = ".$return_status.
    ...
    function getHandleForPID($pid) {
        $stmt = "SELECT rek_handle FROM ". APP_TABLE_PREFIX ."record_search_key_handle WHERE rek_handle_pid = '".$pid."' ";
	$res = $GLOBALS["db_api"]->dbh->getAll($stmt, DB_FETCHMODE_ASSOC);
        if (PEAR::isError($res)) {
	...
	if (sizeof($res) == 0) {
	    Error_Handler::logError(array("No PID found for Handle"), __FILE__, __LINE__);
	    return null;
	} else if (sizeof($res) > 1) {
	    Error_Handler::logError(array("Duplicate PIDs found in Handle search key table - BAD"), __FILE__, __LINE__);
	    return null;
	}		  
    ...
    function isHandleSet($handle) {
      ...
      // This should really query the handle directly as this is flawed as handles could already have been created externally to this fez instance
        ...
        $stmt = "SELECT count(rek_handle_pid) as count FROM ". APP_TABLE_PREFIX ."record_search_key_handle WHERE rek_handle = '".$handle."' ";
	$res = $GLOBALS["db_api"]->dbh->getAll($stmt, DB_FETCHMODE_ASSOC);
        if (PEAR::isError($res)) {
	    Error_Handler::logError(array($res->getMessage(), $res->getDebugInfo()), __FILE__, __LINE__);
	    return null;
    ...
    function processHandleRequests() {
        ...
        if ( $this->processed ) {
  	    Error_Handler::logError(array("This Handle object has already been processed!: ", null, __FILE__, __LINE__));
	    return false;
	}
	...
	if ($this->updateNumber > 0) {
	    if ( file_exists( $updateTmpFile ) ) {
	        Error_Handler::logError(array($updateTmpFile." already exists - possible Handle process in progress or old tmp file not deleted!: ", null, __FILE__, __LINE__));
		return false;
	    } else {
	...
	if ($this->createNumber > 0) {
	    if ( file_exists( $createTmpFile ) ) {
	        Error_Handler::logError(array($createTmpFile." already exists - possible Handle process in progress or old tmp file not deleted!: ", null, __FILE__, __LINE__));
		return false;
	    } else {
	...
	if ($this->deleteNumber > 0) {
	    if ( file_exists( $deleteTmpFile ) ) {
	        Error_Handler::logError(array($deleteTmpFile." already exists - possible Handle process in progress or old tmp file not deleted!: ", null, __FILE__, __LINE__));
		return false;
	    } else {
