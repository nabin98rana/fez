...
class UserComments
{
    ...
    function getUserComments($thepid) {
        ...
        $res = $GLOBALS["db_api"]->dbh->query($stmt, array($thepid), DB_FETCHMODE_ASSOC);
    ...
    function addUserComment($comment, $rating = 0, $usr_id) {
        ...
        $stmt = "INSERT INTO ". APP_TABLE_PREFIX ."user_comments (usc_pid, usc_userid, usc_comment, usc_rating, usc_date_created) " .
                "VALUES (?, ?, ?, ?, ?)";
        $sth = $GLOBALS["db_api"]->dbh->prepare($stmt);
        $res = $GLOBALS["db_api"]->dbh->execute($sth, $comment);
        ...
        $comment['usc_id'] = $GLOBALS["db_api"]->get_last_insert_id();
