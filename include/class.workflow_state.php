...
class Workflow_State
{
	...
	function getDetails($wfs_id)
	{
		...
		$stmt = "SELECT
                 ...
                 WHERE
                    wfs_id=".$db->quote($wfs_id, 'INTEGER');
		try {
	...
	function update($params = array())
	{
		...
		$stmt = "DELETE FROM
                        " . APP_TABLE_PREFIX . "workflow_state_roles
                     WHERE
                        wfsr_wfs_id=" . $db->quote($params['id'],'INTEGER');
		try {
			$db->query($stmt);
		}
		catch(Exception $ex) {
	...
	function remove()
	{
		...
		$stmt = "DELETE FROM
                    " . APP_TABLE_PREFIX . "workflow_state
                 WHERE
                    wfs_id IN (".Misc::arrayToSQLBindStr($_POST["items"]).")";
		try {
	...
	function removeByWorkflow($wfl_ids)
	{
		...
		$stmt = "SELECT wfs_id FROM " . APP_TABLE_PREFIX . "workflow_state" .
                " WHERE wfs_wfl_id IN (".Misc::arrayToSQLBindStr($wfl_ids).")";
		try {
		...
		$stmt = "DELETE FROM
                    " . APP_TABLE_PREFIX . "workflow_state
                 WHERE
                    wfs_wfl_id IN (".Misc::arrayToSQLBindStr($wfl_ids).")";
		try {
	...
	function getAuthRoles($wfs_id) 
	{
		...
		$stmt = "SELECT
                 ...
                 FROM
                    " . APP_TABLE_PREFIX . "workflow_state_roles
                 WHERE
                    wfsr_wfs_id=".$db->quote($wfs_id, 'INTEGER');
		try {
