...
// necessary classes
require_once('class.soapclient_internal.php');
...
function timestamp_to_iso8601($timestamp,$utc=true){
	...
	if($utc){
		...
		if(preg_match($preg_matchStr,$datestr,$regs)){
...
//		}
		...
		if (
			...
			$this->setError("Unsupported HTTP response status $http_status $http_reason (soapclient_internal->response has contents of the response)");
			return false;
		}
	...
	function parse_http_headers() 
	{
		...
		if(function_exists('getallheaders')){
			...
			if(isset($this->headers['content-type']) && strpos($this->headers['content-type'],'=')){
				...
				if(preg_matchi('^(ISO-8859-1|US-ASCII|UTF-8)$',$enc)){
...
/**
*
* soapclient_internal higher level class for easy usage.
*
* usage:
*
* // instantiate client with server info
* $soapclient_internal = new soapclient_internal( string path [ ,boolean wsdl] );
*
* // call method, get results
* echo $soapclient_internal->call( string methodname [ ,array parameters] );
*
* // bye bye client
* unset($soapclient_internal);
*
* @author   Dietrich Ayala <dietrich@ganx4.com>
* @version  $Id: nusoap.php,v 1.95 2006/02/02 15:52:34 snichol Exp $
* @access   public
*/
class soapclient_internal extends nusoap_base  {
	...
	* @access   public
	*/
	function soapclient_internal($endpoint,$wsdl = false,$proxyhost = false,$proxyport = false,$proxyusername = false, $proxypassword = false, $timeout = 0, $response_timeout = 30){
		parent::nusoap_base();
...
//				$this->wsdl =& new wsdl($this->wsdlFile,$this->proxyhost,$this->proxyport,$this->proxyusername,$this->proxypassword,$this->timeout,$this->response_timeout);
	...
	*/
	...
	";
		...
		$evalStr = 'class soap_proxy_'.$r.' extends soapclient_internal {
	'.$evalStr.'
}';
		return $evalStr;
	}
