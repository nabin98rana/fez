<?php
/**
 * NAJAX HTML DOM Base Element file.
 *
 * <p>This file defines the {@link NAJAX_HTML_DOM_BaseElement} Class.</p>
 *
 ...
 *
 * <p>This class is used as base class for all {@link NAJAX_HTML}
 * DOM elements.</p>
 *
 * @author		Stanimir Angeloff
 *
...
class NAJAX_HTML_DOM_BaseElement
{
	...
	/**
	 * Creates a new instance of the {@link NAJAX_HTML_DOM_BaseElement} class.
	 *
	 * @access	public
	 *
	 */
	function NAJAX_HTML_DOM_BaseElement()
	{
	...
	function removeAttribute($attName)
	{
		...
		$this->clientCode .= NAJAX_Client::register($attName) . ');';
	}
	...
	function setAttribute($name, $value)
	{
		...
		$this->clientCode .= NAJAX_Client::register($name) . ',';
		$this->clientCode .= NAJAX_Client::register($value) . ');';
	}
			...
			if (strcasecmp($key, 'attributes') == 0) {
				...
				foreach ($this->attributes as $key => $value) {
					...
					if ($value === null) {
						...
						$returnValue .= $element . '.setAttribute("' . $key . '", ' . NAJAX_Client::register($value) . ');';
					}
				}
				...
				foreach ($this->style as $key => $value) {
					...
					$returnValue .= $element . '.style.' . $key . '=' . NAJAX_Client::register($value) . ';';
				}
				...
				$assignOperation = $assignField . '=' . NAJAX_Client::register($value) . ';';

				if (strpos($assignOperation, NAJAX_HTML_CURRENT_VALUE) !== false) {

					$assignOperation = str_replace(NAJAX_HTML_CURRENT_VALUE, '"+' . $assignField .'+"', $assignOperation);
				}
