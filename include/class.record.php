...
class Record
{
	...
	function getParentsDetails($pid, $clearcache=false, $searchKey='isMemberOf')
	{
		...
		$stmt = "SELECT ".APP_SQL_CACHE."
				 ...
				 FROM
					...
					" . APP_TABLE_PREFIX . "record_search_key_".$sek_title." m1 on r1.rek_pid = m1.rek_".$sek_title." AND m1.rek_".$sek_title."_pid = ".$db->quote($pid);
	...
	function getChildrensDetails($pid, $clearcache=false, $searchKey='isMemberOf')
	{
		...
		$stmt = "SELECT ".APP_SQL_CACHE."
				 ...
				 FROM
					" . APP_TABLE_PREFIX . "record_search_key r1 inner join
					" . APP_TABLE_PREFIX . "record_search_key_".$sek_title." m1 on r1.rek_pid = m1.rek_".$sek_title."_pid AND m1.rek_".$sek_title." = ".$db->quote($pid);
	...
	function removeIndexRecordByXSDMF_ID($pid, $xsdmf_id)
	{
		...
		if ($sekDet['sek_relationship'] == 1) {
			$stmt = "DELETE FROM
					 ...
					 WHERE rek".$sekTableName."_pid = " . $db->quote($pid) . " and rek_".$sekDet['sek_title_db']."_xsdmf_id=".$db->quote($xsdmf_id, 'INTEGER');
			try {
				$db->query($stmt);
			}
			catch(Exception $ex) {
	...
	/*function insertIndexMatchingField($pid, $dsID='', $xsdmf_id, $value)
	{
		...
		if ($sekDet['sek_data_type'] != 'int') { //quote and escape varchar/date/text values
			$value = $db->quote(trim($value));
		}
		else
		$value = $db->quote(trim($value), 'INTEGER');
		...
		if (is_numeric(strpos(APP_SQL_DBTYPE, "mysql"))) {
			...
			if ($sekDet['sek_relationship'] == 0) { //only check for dupes on core 1-1 table inserts as the others could have the same value legitimatly (eg two j smith author strings)
				$stmt = "IF EXISTS( SELECT * FROM " . APP_TABLE_PREFIX . "record_search_key".$sekTableName."
				  ...
				  SET rek_".$sekDet['sek_title_db']."_xsdmf_id = ".$db->quote($xsdmf_id, 'INTEGER').", rek_".$sekDet['sek_title_db']." = ".$value."
				  WHERE rek_pid = ".$db->quote($pid)."
				ELSE
				 ...
				 VALUES
				  (".$db->quote($pid).", ".$db->quote($xsdmf_id, 'INTEGER').", ".$value.")";
			} else {
				$stmt = "INSERT INTO " . APP_TABLE_PREFIX . "record_search_key".$sekTableName."
				 ...
				 VALUES
				  (".$db->quote($pid).", ".$db->quote($xsdmf_id, 'INTEGER').", ".$value.$cardinality_extra_value.")";
			}
		...
		if (APP_SOLR_INDEXER == "ON") {
			$log->debug("Record::insertMatchingField() ADDING ".$pid." TO QUEUE");
	...
	function getAuthWorkflowsByPIDS(&$result, $usr_id)
	{
		...
		if (!Auth::isAdministrator() && (is_numeric($usr_id))) {
			...
			$stmt = "SELECT rek_pid, authi_pid, authi_role, wfl_id, wfl_title, wft_id, wft_icon
	                 ...
	                 INNER JOIN ".$dbtp."auth_rule_group_users ON authi_arg_id = argu_arg_id and argu_usr_id = ".$db->quote($usr_id, 'INTEGER')." 
	                 LEFT JOIN ".$dbtp."record_search_key_assigned_user_id ON rek_pid = rek_assigned_user_id_pid 
	                 LEFT JOIN ".$dbtp."workflow_roles ON authi_role = wfr_aro_id OR (authi_role = 7 AND wfr_aro_id = 8 AND rek_status != 2 AND (rek_assigned_user_id IN (".$db->quote($usr_id, 'INTEGER').") ) )
	...
	function buildSearchKeyJoins($options, $sort_by, $operator, $filter)
	{
		...
		if ($searchKeys["0"]  && trim($searchKeys["0"]) != "") { //this will have to replaced with lots of union select joins like eventum
			...
			if (APP_MYSQL_INNODB_FLAG == "ON" || APP_SQL_DBTYPE != "mysql") {
				...
				$searchKey_join[SK_JOIN] .= $joinType." (SELECT rek_pid, MATCH(rek_pid, rek_title, rek_description) AGAINST (".$db->quote($input).") AS Relevance ".
														" FROM {$dbtp}record_search_key ".
														" WHERE MATCH (rek_pid, rek_title, rek_description) AGAINST (".$db->quote('*'.$input.'*')." IN BOOLEAN MODE)".
														" UNION ".
														" SELECT rek_keywords_pid AS rek_pid, MATCH(rek_keywords) AGAINST (".$db->quote($input).") AS Relevance ".
														" FROM {$dbtp}record_search_key_keywords ".
				...
				if (is_array($searchValue)) {
						...
						if (!empty($searchValue) && $searchValue['filter_enabled'] == 1) {
							...
							switch ($searchValue['filter_type']) {
								...
								case 'less':
									...
									break;
								case 'between':
									$sqlDate = " BETWEEN ".$db->quote($searchValue['start_date'])." AND ".$db->quote($searchValue['end_date']);
									break;
							}
					...
					&& ($sekdet['sek_html_input'] == 'text' || $sekdet['sek_html_input'] == 'textarea')) {
						...
						if ($sekdet['sek_title_db'] == "pid") {
							...
							$searchKey_join["sk_where_$operatorToUse"][] = "$sqlColumnName like ".$db->quote(str_replace("*", "%", $searchValue))." ";
						} else {
							$searchKey_join["sk_where_$operatorToUse"][] = "$sqlColumnName like ".$db->quote('%'.$searchValue.'%')." ";
						}
...
//		Logger::debug("Record::getDatastreamQuickAuthTemplate WITH VALUE ".$userPIDAuthGroups['datastreamQuickAuth']." ");
	...
	public function renameDatastream($pid, $oldName, $newName, $historyDescription = '')
	{
		...
		if (!count($oldDatastream)) {
			$log->err("Could not rename datastream '{$oldName}' to '{$newName}' in {$pid} because the original datastream doesn't exist");
			return false;
		}
	...
	protected function renameDatastreamInternal($pid, $ds, $newName)
	{
		...
		switch ($ds['controlGroup']) {
			...
			case 'X':
				...
				$result = Fedora_API::getUploadLocation($pid, $newName, $value, $ds['label'], $ds['MIMEType'], $ds['controlGroup'], null, $ds['versionable']);
				break;
			case 'R':
				Fedora_API::callAddDatastream($pid, $newName, $ds['location'], $ds['label'], $ds['state'],$ds['MIMEType'], $ds['controlGroup'], $ds['versionable']);
		...
		if (!$result) {
			$log->err("Could not rename datastream '{$ds['ID']}' to '{$newName}' in {$pid} because we could not purge the old datastream");
		}
