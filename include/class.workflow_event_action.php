...
class Workflow_Event_Action
{
     ...
     */
    function getDetails($wfa_id)
    {
        $stmt = "SELECT
                    *
                 FROM
                    " . APP_DEFAULT_DB . "." . APP_TABLE_PREFIX . "workflow_event_action
                 WHERE
                    wfa_id=$wfa_id";
    ...
    function insert()
    {
        ...
        $stmt = "INSERT INTO
                 ...
                 (
                    ...
                    " . $HTTP_POST_VARS['wfe_id'] . ",
                    " . $HTTP_POST_VARS['wfa_wfb_id'] . ",
                    '" . Date_API::getCurrentDateGMT() . "',
                    " . $HTTP_POST_VARS['wfa_rank'] . "
                 )";
		echo $stmt;
  ...
  /**
    ...
    function update()
    {
        ...
        $stmt = "UPDATE
                    " . APP_DEFAULT_DB . "." . APP_TABLE_PREFIX . "workflow_event_action
                 SET
                    wfa_last_updated_date='" . Date_API::getCurrentDateGMT() . "',
                    wfa_rank=" . $HTTP_POST_VARS['wfa_rank'] . ",
                    wfa_wfb_id=" . $HTTP_POST_VARS['wfa_wfb_id'] . "
                 WHERE
                    wfa_id=" . $HTTP_POST_VARS['id'];
        $res = $GLOBALS["db_api"]->dbh->query($stmt);
        if (PEAR::isError($res)) {
            Error_Handler::logError(array($res->getMessage(), $res->getDebugInfo()), __FILE__, __LINE__);
            return -1;
...
/*            // remove any user list associated with this reminder action
     ...
     * @return  array The list of reminder actions
     */
    function getList($wfe_id)
    {
        $stmt = "SELECT
                    *
                 FROM
                    " . APP_DEFAULT_DB . "." . APP_TABLE_PREFIX . "workflow_event_action left join
                    " . APP_DEFAULT_DB . "." . APP_TABLE_PREFIX . "wfbehaviour on (wfa_wfb_id = wfb_id)
                 WHERE
                    wfa_wfe_id=$wfe_id 
                 ORDER BY
                    wfa_rank ASC";
