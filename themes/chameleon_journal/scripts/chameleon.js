...
(function() {
    ...
    var CSS = {
        ...
        clearTheme: function() {
            ...
            var n = links.length;
            while (n--) {
                if (links[n].href && links[n].href.indexOf('/moodle/theme/chameleon/styles.php') != -1) {
        ...
        loadRemote: function(doSetup) {
            ...
            xmlhttp.open('GET', document.getElementById("cssLocation").firstChild.nodeValue , true);
            xmlhttp.send(null);
            return true;
        },
        
        loadDefault: function(doSetup) {
        if (!Sarissa.IS_ENABLED_XMLHTTP) {
        return false;
        }
        var xmlhttp = new XMLHttpRequest();
        ...
        UI.statusMsg('Default stylesheet....loading');
        CSS.__remoteCSS = CSS.toObject(xmlhttp.responseText);
        CSS.__localCSS = CSS.__clone(CSS.__remoteCSS);
        CSS.preview();
        if (doSetup) {
        ...
        xmlhttp.open('GET', document.getElementById("defaultCssLocation").firstChild.nodeValue , true);
        xmlhttp.send(null);
        return true;
        },     
        ...
        updateRemote: function() {
            ...
            xmlhttp.open('POST', document.getElementById("cssUploadUri").firstChild.nodeValue , true);
    ...
    imageUpload: function(e, noPrompt) {
    var themeName = document.getElementById("themeName").firstChild.nodeValue;
    var imageUploadUri = document.getElementById("imageUploadUri").firstChild.nodeValue;
    {
    var fUpload = window.open ('', 'FileUpload', 'height=150,width=400, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no, directories=no, status=no');
    fUpload.document.write( "<html><head><title>File Upload Module</title>");
    fUpload.document.write( "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=iso-8859-1\" />");
    fUpload.document.write("<link type='text/css' rel='stylesheet' href='" + document.getElementById("cssLocation").firstChild.nodeValue + "' />");
    fUpload.document.write( "<script language='javascript'>function upload(){setTimeout(document.iform.submit(),5000);}</script>");
    fUpload.document.write( "</head><body>");
    fUpload.document.write( "<h3>select your file</h3><br/>");
    fUpload.document.write("<form id='iform' name='iform' action='" + imageUploadUri + "' method='POST' enctype='multipart/form-data'>");
    fUpload.document.write("<input id='ifile' type='file'  name='ifile' onchange='upload()' />");
    fUpload.document.write("<input id='themeBase' name='themeBase' type='hidden'  value='" + themeName + "'/>");
    fUpload.document.write("</form>");
    fUpload.document.write( "</body></html>");
    }
    },     
        ...
        fixPath: function(val) {
            ...
            var tmp = val.split('(');
            if (tmp.length > 1) {
            if(tmp[1].substring(0,2) == '..'){
            tmp[1] = document.getElementById("themeRoot").firstChild.nodeValue + tmp[1].substring(2);
            }
            else{
            tmp[1] = document.getElementById("themeRoot").firstChild.nodeValue + '/' + tmp[1];
            }
             return tmp.join('(');
            }
            return document.getElementById("themeRoot").firstChild.nodeValue + '/' + val;
        },
    ...
    CSS.__Shorthand = {
        ...
        setBackground: function(css, selector, value) {
            ...
            css[selector]['background-image'] = 'url(' + bits[1] + ')'; //"
            
            var pos = [];
          //  alert("bg-img" + css[selector]['background-image']);
    ...
    var FileHandler = {
        ...
        getFiles: function(path) {
            ...
            xmlhttp.open('GET', document.getElementById("imageLocation").firstChild.nodeValue + '?nc=' + new Date().getTime(), true);
            xmlhttp.send(null);
            return true;
        }
    };
    ...
    UI.Selector = {
        ...
        __editCode: function() {
            ...
            UI.setupButtons('chameleon-selector-buttons', 'revert', 'save-server');
    ...
    UI.CSS = {
        ...
        __setImageDisplay: function(value, field, picker) {
            ...
            //    alert("value " + value);
                if (field.value != 'none') {
                preview.setAttribute('src', CSS.fixPath(value));
                } else {
        ...
        __addButtons: function() {
            ...
            p.style.textAlign = 'right';
            p.appendChild(UI.createButton('chameleon-style-buttons-upload', 'Upload_Image', 'Upload Image to Server', CSS.imageUpload));
        ...
        displayImagePicker: function(xmldata) {
            UI.clearStatusMsg();
//alert("trying to display imagepicker");
