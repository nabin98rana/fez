<?php /* Smarty version 2.6.2, created on 2005-05-06 16:22:54
         compiled from manage/collections.tpl.html */ ?>
<?php require_once(SMARTY_DIR . 'core' . DIRECTORY_SEPARATOR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('function', 'cycle', 'manage/collections.tpl.html', 105, false),array('modifier', 'capitalize', 'manage/collections.tpl.html', 112, false),)), $this); ?>
            ...
            <table bgcolor="#FFFFFF" width="100%" cellspacing="1" cellpadding="2" border="0">
              ...
              function validateForm(f)
              {
                  ...
                  if (isWhitespace(f.title.value)) {
                      alert(\'Please enter the title of this collection.\');
                      selectField(f, \'title\');
                      return false;
                  }
                  if (!hasOneSelected(f, \'users[]\')) {
                      alert(\'Please assign the users for this collection.\');
                      selectField(f, \'users[]\');
                      return false;
                  }
                  if (!hasOneSelected(f, \'statuses[]\')) {
                      alert(\'Please assign the statuses for this collection.\');
                  ...
                  if (!found) {
                      alert(\'Please choose the initial status from one of the assigned statuses of this collection.\');
                      selectField(f, \'initial_status\');
                      return false;
                  }
                  if (isWhitespace(f.outgoing_sender_email.value)) {
                      alert(\'Please enter the outgoing sender address for this collection.\');
                ...
                <td colspan="2">
                  ...
                  function checkDelete(f)
                  {
                      ...
                      if (total == total_selected) {
                          alert(\'You cannot remove all of the collections in the system.\');
                          return false;
                      }
                      if (!hasOneChecked(f, \'items[]\')) {
                          alert(\'Please select at least one of the collections.\');
                          return false;
                      }
                      if (!confirm(\'WARNING: This action will remove the selected collections permanently.\\nIt will remove all of its associated entries as well (issues, notes, attachments,\\netc), so please click OK to confirm.\')) {
                          return false;
                      } else {
                          return true;
                      }
...
" class="default_white">&nbsp;<b>Title</b></td>
                      <td bgcolor="<?php echo $this->_tpl_vars['cell_color']; ?>
" class="default_white">&nbsp;<b>Primary Collection Manager</b></td>
                      <td bgcolor="<?php echo $this->_tpl_vars['cell_color']; ?>
