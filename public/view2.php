...
if (!empty($pid) && $record->checkExists()) {
  ...
  if ($xdis_title == 'Journal Article') {
    $sub_type = Record::getSearchKeyIndexValue($pid, "Subtype", $lookup);
  } elseif ($xdis_title == 'Conference Paper') {
    $sub_type = Record::getSearchKeyIndexValue($pid, "Genre Type", $lookup);
  } else {
    $sub_type = false;
  }
  if (APP_FEDORA_BYPASS == 'ON') {
  ...
  if (empty($details)) {
    ...
    if (count($oa_status) == 1 && array_values($oa_status)[0] == 'Link (no DOI)') {   //We just want the value not key of the one element array $oa_status
      $linkCount++;
