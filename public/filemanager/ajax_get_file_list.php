...
?><table class="tableList" id="tableList" cellpadding="0" cellspacing="0">
					<thead>
						<tr>
							<th width="5%"><a href="#" class="check_all" id="tickAll" title="Select All" onclick="checkAll('Select All', 'Unselect All');">&nbsp;</a></th>
							<th width="6%" class="center">&nbsp;</th>
							<th width="48%" class="left">Name</th>
							<th width="10%" class="center">Size</th>
							<th width="31%" class="center">Modified At</th>
						</tr>
					</thead>
					<tbody id="fileList">
<tr class="even" id="topRow" onclick="setDocInfo('folder', '0');">
							<td><input type="checkbox" name="check[]" id="check0" disabled="disabled"  />
								...
								<input type="hidden" name="folderCtime0" id="folderCtime0" value="<?php echo date('d/M/Y H:i:s',$folderInfo['ctime']); ?>" />
								<input type="hidden" name="folderMtime0" id="folderMtime0" value="<?php echo date('d/M/Y H:i:s',$folderInfo['mtime']); ?>" />
							...
							foreach($fileList as $file)
							{
								...
								<tr class="<?php echo $css; ?>" id="row<?php echo $count; ?>"  >
									<td onclick="setDocInfo('<?php echo $file['type']; ?>', '<?php echo $count; ?>');"><input type="checkbox" name="check[]" id="check<?php echo $count; ?>" value="<?php echo $file['name']; ?>" <?php echo $strDisabled; ?> />
										...
										<input type="hidden" name="fileCtime<?php echo $count; ?>" id="fileCtime<?php echo $count; ?>" value="<?php echo date('d/M/Y H:i:s',$file['ctime']); ?>" />
										<input type="hidden" name="fileMtime<?php echo $count; ?>" id="fileMtime<?php echo $count; ?>" value="<?php echo date('d/M/Y H:i:s',$file['mtime']); ?>" />
									...
									<td><span class="<?php echo $file['cssClass']; ?>"><span id="flag<?php echo $count; ?>" class="<?php echo $file['flag']; ?>">&nbsp;</span></span></td>
								...
								}else
								{
									...
									<tr class="<?php echo $css; ?>" id="row<?php echo $count; ?>" >
										<td onclick="setDocInfo('folder', '<?php echo $count; ?>');"><input type="checkbox" name="check[]" id="check<?php echo $count; ?>" value="<?php echo $file['name']; ?>" <?php echo $strDisabled; ?>/>
											...
											<input type="hidden" name="folderCtime<?php echo $count; ?>" id="folderCtime<?php echo $count; ?>" value="<?php echo date('d/M/Y H:i:s',$file['ctime']); ?>" />
											<input type="hidden" name="folderMtime<?php echo $count; ?>" id="folderMtime<?php echo $count; ?>" value="<?php echo date('d/M/Y H:i:s',$file['mtime']); ?>" />
