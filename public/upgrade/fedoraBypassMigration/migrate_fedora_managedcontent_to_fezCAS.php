...
foreach ($fedoraPids as $pid) {
    ...
    foreach ($datastreams as $datastream) {
        if ($datastream['controlGroup'] == 'M') {
            ...
            // Log save status
            if (!$result) {
                echo "ERROR: "." UNABLE TO SAVE $pid $dsid ".APP_DSTREE_PATH.$fileHash['hashPath'].$fileHash['hashFile']."\n";
            ...
            if (!$acmlBase ) {
                ...
                echo $did ." ".$pid." ".$datastream['ID']." ".APP_DSTREE_PATH.$fileHash['hashPath'].$fileHash['hashFile']." (no DS ACML found) <br/>\n";
            } else
            {
                addDatastreamSecurity($acml, $pid, $datastream['ID']);
                $did = AuthNoFedoraDatastreams::getDid($pid, $dsID);
                echo $did ." ".$pid." ".$datastream['ID']." ".APP_DSTREE_PATH.$fileHash['hashPath'].$fileHash['hashFile']." (found DS ACML so adding) <br/>\n";
            }
        }
    }
}
