	...
	function getDetails()
	{
						...
						if (is_array($details[$dis_field['xsdmf_id']])) {
							foreach ($details[$dis_field['xsdmf_id']] as $ckey => $cdata) {
								...
								if ($dis_field['sek_title'] == "Subject" || $dis_field['sek_title'] == "Fields of Research" || $dis_field['sek_title'] == "SEO Code") {
									...
									$details[$dis_field['xsdmf_id']][$ckey] = "<a title=".'"'."Search the repository for more like this".'"'." class='silent_link' href=".'"'.APP_BASE_URL."list/?cat=quick_filter&amp;search_keys%5B".$dis_field['xsdmf_sek_id']."%5D=".urlencode($details[$dis_field['xsdmf_id']][$ckey]).'"'.">".$cdata."</a>";
								}
							}
						} else {
							if ($dis_field['sek_title'] == "Subject" || $dis_field['sek_title'] == "Fields of Research" || $dis_field['sek_title'] == "SEO Code") {
        ...
        if ($dis_field['xsdmf_html_input'] == "rich_text") {
          $details[$dis_field['xsdmf_id']] = strip_tags($details[$dis_field['xsdmf_id']], '<p><br><b><i><u><strong><sub><sup><em>');
				...
				if ($dis_field['sek_title'] == "Author" || $dis_field['sek_title'] == "Contributor" || $dis_field['sek_title'] == "Creator Name") {
					if (!empty($details[$dis_field['xsdmf_id']])) {
						if (is_array($details[$dis_field['xsdmf_id']])) {
							foreach ($details[$dis_field['xsdmf_id']] as $ckey => $cdata) {
				...
				if ($dis_field['sek_title'] == "Keywords") {
					if (!empty($details[$dis_field['xsdmf_id']])) {
						if (is_array($details[$dis_field['xsdmf_id']])) {
							foreach ($details[$dis_field['xsdmf_id']] as $ckey => $cdata) {
								$details[$dis_field['xsdmf_id']][$ckey] = "<a class='silent_link' href=".'"'.APP_BASE_URL."list/?cat=quick_filter&amp;search_keys%5B0%5D=".urlencode($details[$dis_field['xsdmf_id']][$ckey]).'"'.">".htmlspecialchars($details[$dis_field['xsdmf_id']][$ckey])."</a>";
							}
						} else {
							$details[$dis_field['xsdmf_id']] = "<a class='silent_link' href=".'"'.APP_BASE_URL."list/?cat=quick_filter&amp;search_keys%5B0%5D=".urlencode($details[$dis_field['xsdmf_id']]).'"'.">".htmlspecialchars($details[$dis_field['xsdmf_id']])."</a>";
						}
					}
				}
				...
				if ($dis_field['sek_title'] == "Journal Name" || $dis_field['sek_title'] == "Proceedings Title") {
					if (!empty($details[$dis_field['xsdmf_id']])) {
						...
						if (is_array($details[$dis_field['xsdmf_id']])) {
							foreach ($details[$dis_field['xsdmf_id']] as $ckey => $cdata) {
								$details[$dis_field['xsdmf_id']][$ckey] = "<a class='silent_link' href=".'"'.APP_BASE_URL."list/?cat=quick_filter&amp;search_keys%5B".$dis_field['xsdmf_sek_id']."%5D=".urlencode($details[$dis_field['xsdmf_id']][$ckey]).'"'.">".htmlspecialchars($details[$dis_field['xsdmf_id']][$ckey])."</a>".$rjl;
							}
						} else {
							$details[$dis_field['xsdmf_id']] = "<a class='silent_link' href=".'"'.APP_BASE_URL."list/?cat=quick_filter&amp;search_keys%5B".$dis_field['xsdmf_sek_id']."%5D=".urlencode($details[$dis_field['xsdmf_id']]).'"'.">".htmlspecialchars($details[$dis_field['xsdmf_id']])."</a>".$rjl;
						}
					}
				}
				...
				if ($dis_field['sek_title'] == "Conference Name") {
					if (!empty($details[$dis_field['xsdmf_id']])) {
						...
						if (is_array($details[$dis_field['xsdmf_id']])) {
							foreach ($details[$dis_field['xsdmf_id']] as $ckey => $cdata) {
								$details[$dis_field['xsdmf_id']][$ckey] = "<a class='silent_link' href=".'"'.APP_BASE_URL."list/?cat=quick_filter&amp;search_keys%5B".$dis_field['xsdmf_sek_id']."%5D=".urlencode($details[$dis_field['xsdmf_id']][$ckey]).'"'.">".htmlspecialchars($details[$dis_field['xsdmf_id']][$ckey])."</a>".$rcl;
							}
						} else {
							$details[$dis_field['xsdmf_id']] = "<a class='silent_link' href=".'"'.APP_BASE_URL."list/?cat=quick_filter&amp;search_keys%5B".$dis_field['xsdmf_sek_id']."%5D=".urlencode($details[$dis_field['xsdmf_id']]).'"'.">".htmlspecialchars($details[$dis_field['xsdmf_id']])."</a>".$rcl;
						}
					}
				}
				...
				if (($dis_field['sek_title'] == "Subject"  || $dis_field['sek_title'] == "Fields of Research" || $dis_field['sek_title'] == "SEO Code" || $dis_field['sek_title'] == "Book Title" || $dis_field['sek_title'] == "Job Number" || $dis_field['sek_title'] == "Series") && (($dis_field['xsdmf_html_input'] != "contvocab_selector")) ) {
					if (!empty($details[$dis_field['xsdmf_id']])) {
						if (is_array($details[$dis_field['xsdmf_id']])) {
							foreach ($details[$dis_field['xsdmf_id']] as $ckey => $cdata) {
								$details[$dis_field['xsdmf_id']][$ckey] = "<a class='silent_link' href='".APP_BASE_URL."list/?cat=quick_filter&amp;search_keys%5B".$dis_field['xsdmf_sek_id']."%5D=".htmlspecialchars($details[$dis_field['xsdmf_id']][$ckey])."'>".htmlspecialchars($details[$dis_field['xsdmf_id']][$ckey])."</a>";
							}
						} else {
							$details[$dis_field['xsdmf_id']] = "<a class='silent_link' href='".APP_BASE_URL."list/?cat=quick_filter&amp;search_keys%5B".$dis_field['xsdmf_sek_id']."%5D=".htmlspecialchars($details[$dis_field['xsdmf_id']])."'>".htmlspecialchars($details[$dis_field['xsdmf_id']])."</a>";
						}
					}
				}
					...
					foreach ($parent_relationships as $pkey => $prel) {
						if (in_array($dis_field["xsdmf_parent_option_xdis_id"], $prel)) {
							...
							if (is_array($parent_details[$dis_field["xsdmf_parent_option_child_xsdmf_id"]])) {
								...
								if ($xsdmf_details['xsdmf_smarty_variable'] != "" && ($xsdmf_details['xsdmf_html_input'] == "multiple" || $xsdmf_details['xsdmf_html_input'] == "dual_multiple" || $xsdmf_details['xsdmf_html_input'] == "pid_selector")) {
