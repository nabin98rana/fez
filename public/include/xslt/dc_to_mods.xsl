...
<!--
	...
	-->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:sru_dc="info:srw/schema/1/dc-schema" xmlns:oai_dc="http://www.openarchives.org/OAI/2.0/oai_dc/" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns="http://www.loc.gov/mods/v3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" exclude-result-prefixes="sru_dc oai_dc dc" version="1.0" xmlns:mods="http://www.loc.gov/mods/v3">
	...
	<xsl:template match="dc:subject">
		<subject>
			...
			<xsl:attribute name="authority">asrc</xsl:attribute>
			<topic>
				<xsl:apply-templates/>
			</topic>
		</subject>
	...
	<xsl:template match="dc:type"><!-- based on DCMI Type Vocabulary as of 2006-10-27 at http://dublincore.org/documents/dcmi-type-vocabulary/ ... see also the included dcmiType.xsl serving as variable $types -->
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:if test="not(string($types) = text())">
					<xsl:variable name="lowercaseType" select="translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz')"/><!--<typeOfResource>
	...
	<xsl:template match="dc:identifier">
		...
		<identifier>
			<xsl:attribute name="type">
				<xsl:choose><!-- handled by location/url -->
					...
					</xsl:when><!-- will need to update for ISBN 13 as of January 1, 2007, see XSL tool at http://isbntools.com/ -->
					...
					<xsl:when test="starts-with(text(), 'LCCN') or starts-with(text(), 'lccn')"><!-- probably can't do this quickly or easily without regexes and XSL 2.0 -->
	...
	<xsl:template match="dc:coverage">
		<xsl:choose>
			<xsl:when test="string-length(text()) &gt;= 3 and (starts-with(text(), &apos;1&apos;) or starts-with(text(), &apos;2&apos;) or starts-with(text(), &apos;3&apos;) or starts-with(text(), &apos;4&apos;) or starts-with(text(), &apos;5&apos;) or starts-with(text(), &apos;6&apos;) or starts-with(text(), &apos;7&apos;) or starts-with(text(), &apos;8&apos;) or starts-with(text(), &apos;9&apos;) or starts-with(text(), &apos;-&apos;) or contains(text(), &apos;AD&apos;) or contains(text(), &apos;BC&apos;)) and not(contains(text(), &apos;:&apos;))"><!-- using XSL 2.0 for date parsing is A Better And Saner Idea -->
				<subject>
					<temporal>
						<xsl:apply-templates/>
					</temporal>
			...
			<xsl:when test="contains(text(), 'A') or contains(text(), 'geo:lat') or contains(text(), 'geo:lon') or contains(text(), ' N ') or contains(text(), ' S ') or contains(text(), ' E ') or contains(text(), ' W ')"><!-- predicting minutes and seconds with ' or " might break if quotes used for other purposes exist in the text node -->
				<subject>
					<cartographics>
						<coordinates>
							<xsl:apply-templates/>
