...
// +----------------------------------------------------------------------+
// | Copyright (c) 2005 - 2011 The University of Queensland,              |
...
// |          Rhyz Palmer <r.rpalmer@library.uq.edu.au>                   |
// +----------------------------------------------------------------------+
...
class SanityChecks
{
    ...
    function extensions()
    {
        ...
        ob_end_clean();
        if (!preg_match("/mod_rewrite/U", $contents) && !preg_match("/nginx/U", $contents)) { // check for apache mod_rewrite.. unless you are using nginx/php-fpm
            $results[] = new ConfigResult('Apache modules', 'mod_rewrite', '', "The mod_rewrite module needs to be enabled in your Apache configuration file (httpd.conf) in order for Fez to work properly. To enable with Apache 2.2 you would run 'a2enmod rewrite'. You will need to restart Apache after enabling this extension. This should only flag if you are using Apache - rather than Nginx (which is another valid option)");
        }
    ...
    function dirs()
    {
        ...
        if (APP_SAN_IMPORT_DIR) $results = array_merge($results, SanityChecks::checkDir('APP_SAN_IMPORT_DIR', APP_SAN_IMPORT_DIR));
