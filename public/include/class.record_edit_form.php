...
//                        if (!empty($details[$dis_field['xsdmf_id_ref']])) {
            ...
            if (is_array($details[$dis_field['xsdmf_id']])) {
              foreach ($details[$dis_field['xsdmf_id']] as $ckey => $cdata) {
                if (!empty($cdata)) {
                  if ($xsdmf_details_ref['xsdmf_cvo_save_type'] == 0) {
    ...
    if ($datastreams) {
      foreach ($datastreams as $ds_key => $ds) {
        ...
        if ($datastreams[$ds_key]['controlGroup'] == 'R' && $datastreams[$ds_key]['ID'] != 'DOI') {
          ...
          $tempEmbargoDate = Datastream::getEmbargoDate($pid, $datastreams[$ds_key]['ID']);
          $datastreams[$ds_key]['embargoDate'] = empty($tempEmbargoDate) ? '' : date("d-m-Y", strtotime($tempEmbargoDate));
          $fileCount++;
        }
