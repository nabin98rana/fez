...
public static function multi_implode($glue, $pieces)
 {
     ...
     if (is_object($pieces)){
         return str_replace(chr(10), "", print_r($pieces, true));
     }
 ...
 public static function smart_ucwords($string, $upper_all_length = false) {
  ...
  /**
   * Method to return a response in JSON format
   * Inspired by https://github.com/symfony/http-foundation/blob/master/JsonResponse.php
   *
   * @param string $data The string response
  ...
  public static function jsonResponse($data, $callback = null)
  {
    ...
    if (null !== $callback) {
      ...
      foreach ($parts as $part) {
        if (!preg_match($pattern, $part)) {
          $log->err(array("'The callback name is not valid.'",__FILE__,__LINE__));
          return false;
        }
      }
    }
    ...
    if (JSON_ERROR_NONE !== json_last_error()) {
      if (function_exists('json_last_error_msg')) {
        ...
        $error = self::transformJsonError();
      }
      $log->err(array($error, __FILE__, __LINE__));
      return false;
    }
    ...
    if (null !== $callback) {
      ...
      $response = sprintf('/**/%s(%s);', $callback, $data);
    } else {
                    ...
                    switch ($ematch['xsdmf_html_input']) {
                      ...
                      case "depositor_org":
                        $node_label .= '</a> <a target="basefrm" href="'.$match_form_url.$ehref.'&xsdsel_id='.
                          ...
                          break;
                      case "loc_capture_widget":
                          $node_label .= '</a> <a target="basefrm" href="'.$match_form_url.$ehref.'&xsdsel_id='.
                            $ematch["xsdmf_xsdsel_id"].'" class="form_note"> <span class="form_note">'.$disabled_msg.
                            '<b>Location capture widget:</b> '.$ematch['xsdmf_title'].'<br/>Loop: '.
                            $ematch['xsdsel_title'].'<br/>Order: '.$ematch['xsdmf_order'].'<br/>XSDMF ID: '.
                            $ematch['xsdmf_id'].'</span><img src="'.APP_RELATIVE_URL.'images/text_16.png" />';
                           break;
                      default:
                          break;
                    }
  ...
  public static function shortFilename($filename, $maxlen = 20)
  {
