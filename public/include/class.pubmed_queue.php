...
include_once(APP_INC_PATH . "class.bgp_pubmed.php");
include_once(APP_INC_PATH . "class.eventum.php");
include_once(APP_INC_PATH . "class.date.php");
include_once(APP_INC_PATH . "class.pubmed_service.php");
...
class PubmedQueue extends Queue
{
	/**
	...
	public static function get()
	{
		...
		try
		{
			$instance = Zend_Registry::get('Pubmed');
		}
		catch(Exception $ex)
		{
			// Create a new instance
			$instance = new PubmedQueue();
			$instance->_dbtp = APP_TABLE_PREFIX . 'pubmed_';
	...
	public function bgProcess()
	{
		...
		//perform checking for existing record in Fez by:
			//stripping the title and searching fez
			//checking if the issue exists in Fez
			//checking that the start and end page doesn't exist in Fez
			
		//if a suitably indicative combination of the above indicates that we already have this record in Fez
			//check what fields that have data in the Pubmed record
			//are empty in the Espace record and fill in the gaps - 
			//do not replace existing data, only add what is missing
			// - PubmedRecItem->update()
		//else
			//add the record to Fez - PubmedRecItem->save()
	}
	
	//$matchCriteria is an array containing the Entrez record
	//fields that the match is to be performed on. The keys
	//are to be the field names.
	public function getMatchingLocalRecord($matchCriteria)
	{
		//
	}
}

class xPubmedQueue extends Queue
{
  protected $_bgp;
