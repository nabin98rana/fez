...
class User
{
	...
	function updateFullName($usr_id)
	{
      ...
      $stmt = "UPDATE
                    " . APP_TABLE_PREFIX . "user
                 SET
                    usr_full_name=" . $db->quote($full_name) . "
                 WHERE
                    usr_id=".$db->quote($usr_id, 'INTEGER');
		try {
			$db->exec($stmt);
	...
	function updateLoginDetails($usr_id)
	{
		...
		$stmt = "UPDATE
                 ...
                 SET
					...
					usr_real_last_login_date=" . $db->quote($user['usr_last_login_date']) . "
                 WHERE
                    usr_id=".$db->quote($usr_id, 'INTEGER');
		try {
			$db->query($stmt);
...
//        $username = $account['id'];
    ...
    function getUserInfoByRid($rid) {
        $log = FezLog::get();
        $db = DB_API::get();

        $stmt = "SELECT * FROM " . APP_TABLE_PREFIX . "author WHERE aut_researcher_id=".$db->quote($rid)." LIMIT 1";
