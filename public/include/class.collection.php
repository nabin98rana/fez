...
class Collection
{
  ...
  function getAuthIndexStmt($roles = array(), $joinPrefix="r2.rek_pid")
  {
    ...
    $lister_only = false;
    // If lister only use the faster lister only auth index for the inner joins
    if (count($roles) == 1 && $roles[0] == 9) {
      $lister_only = true;
    }
    $dbtp =  APP_TABLE_PREFIX;
    $authStmt = "";
    ...
    if (is_numeric($usr_id)) {
        ...
        if (count($ses['auth_index_user_rule_groups']) > 0) {
          if (!$lister_only) {
            $authStmt .= " INNER JOIN ".$dbtp."auth_index2 ai
                          ...
                          " AND ai.authi_arg_id in (".implode(",", $ses['auth_index_user_rule_groups']).")";
          } else {
            $authStmt .= " INNER JOIN ".$dbtp."auth_index2_lister ai
                          ON ai.authi_pid = ".$joinPrefix." AND ai.authi_arg_id in (".
                          implode(",", $ses['auth_index_user_rule_groups']).")";
          }
        }
      } else {
        if ($lister_only) {
      ...
      if (is_array($publicGroups)) { // to catch when the fez index/auth index is empty
        if (count($publicGroups) > 0) {
          $authStmt = " INNER JOIN ".$dbtp."auth_index2_lister ON authi_pid=".$joinPrefix." and authi_arg_id in (".
                      implode(",", $publicGroups).")";
        }
      }
      $joinStmt .= "";
    }
...
//    $sekdet = Search_Key::getDetailsByTitle($sort_by);
    ...
    if ($searchKey == "Subject") {
      ...
      if (!empty($letter)) {
        $letter = addslashes($letter);
        $letter_restrict = "WHERE r2.rek_".$sekdet['sek_title_db']." LIKE " . $db->quote($letter.'%') . " OR r".
   ...
   * @return  array The list of records
   */
  function statsByAttribute(
      $current_row = 0, $max = 50, $searchKey="Author", $year = "all", $month = "all", $range = "all"
  )
  {
    $log = FezLog::get();
    $db = DB_API::get();
   ...
   * @return  array The list of records
   */
  function statsByAuthorID(
      $current_row = 0, $max = 50, $searchKey="Author ID", $year = "all", $month = "all", $range = "all"
  )
  {
    $log = FezLog::get();
    $db = DB_API::get();
  ...
  function suggest($terms, $current_row = 0, $max = 10) 
  {
    ...
    $stmt = "SELECT ".APP_SQL_CACHE."
                    ...
                    1,char_length(substring_index(concat(trim(substring(r2.rek_title, instr(r2.rek_title, ".
                    $db->quote(' '.$terms)."))),  ' '),
