...
class Record
{
   ...
   */
    ...
    //        }
            ...
            try {
                ...
                $db->rollBack();
                $log->err("stmt: ".$stmt. " Exception - ".$ex->getMessage());
                $ret = false;
            }
        } elseif ($shadow) {
            ...
            foreach ($sekData[1] as $sek_table => $sek_value) {
                ...
                if (!is_array($existingData) || !isset($existingData[0]['rek_'.$sek_table]) || count($existingData[0]['rek_'.$sek_table]) != count($sek_value['xsdmf_value']) || count($diff) > 0) {
                    ...
                    $sekValTest = (is_array($sek_value['xsdmf_value']))
                      ...
                      : $sek_value['xsdmf_value'];
                    if (!empty($sek_value['xsdmf_value']) && !is_null($sek_value['xsdmf_value'])
                      && ($sekValTest != "NULL")
                    ) {
                        $xsdDetails = XSD_HTML_Match::getDetailsByXSDMF_ID($sek_value['xsdmf_id']);
                        $searchKeyDetails = Search_Key::getDetails($xsdDetails['xsdmf_sek_id']);
   ...
   */
    public static function getListing(
        ...
        $doExactMatch = false,
        $facet_limit = APP_SOLR_FACET_LIMIT,
        $facet_mincount = APP_SOLR_FACET_MINCOUNT,
        $getAuthorMatching = false,
        $versionDate = null,
        $forceLocal = false,
        ...
        if ($total_rows > 0) {
            ...
            try {
                ...
                $log->err($ex->getMessage(). " stmt = ".$stmt);
                $res = array();
            }
   ...
   */
    function getSearchListing(
        ...
        if ($sort_by == "score") {
            $sort_by = "searchKey0";
        } elseif (!is_numeric(strpos($sort_by, "searchKey"))) {
            $sort_by_id = Search_Key::getID($sort_by);
  ...
  //$limitedLookupTo restrict lookup to only certain searchKeys
    ...
    function getAuthWorkflowsByPIDS(&$result, $usr_id)
    {
        ...
        } elseif (!is_numeric($usr_id)) { // no workflows for a non-logged in person -
...
//param $previousToDate will return the version previous
    ...
    function buildSearchKeyJoins($options, $sort_by, $operator, $filter)
    {
        ...
        || (is_array($searchKey_join['sk_where_OR']) && count($searchKey_join['sk_where_OR']) > 0) ) {
            ...
            if (is_array($searchKey_join['sk_where_OR']) && count($searchKey_join['sk_where_OR']) > 0) {
                if ($sk_where_and) {
            ...
            for ($i = 0, $j = count($lookups[0]); $i < $j; ++$i) {
                ...
                if (!empty($sekDetails)) {
                    if ($sekDetails['sek_data_type'] == 'int' && $sekDetails['sek_lookup_id_function'] != '') {
                        ...
                        if (!empty($temp_value)) {
                            $escapedInput = str_replace(
                                $lookups[0][$i],
                                $lookups[1][$i] . ":" . $temp_value,
                                $escapedInput
                            );
                        }
                    }
                ...
                if (!empty($searchValue)) {
                    ...
                    if (is_array($searchValue)) {  // Array was submitted for this search key
                        ...
                        && ($sekdet['sek_html_input'] == 'text' || $sekdet['sek_html_input'] == 'textarea')
                            ...
                            $searchKey_join[SK_SEARCH_TXT] .= $sekdet['sek_title'] . ":\"" . htmlspecialchars(trim($searchValue)) . "\", ";
                        } elseif ($sekdet['sek_html_input'] == 'combo') { //combo boxes are exact searches
    ...
    function getSolrSuffix($sek_det, $sort = 0, $facet = 0)
    {
        ...
        if (($sek_data_type == 'int') && ($sek_cardinality == 0)) {
            ...
            if ($sort == 1) {
                $suffix .= "_s";
            } elseif (($facet == 1) || ($sek_det['sek_html_input'] == "combo")) { //Combo boxes should be exact
                $suffix .= "_ft";
            }
   ...
   *
   */
    public function insertFromArray(
        $mods,
        $rels_parent_pid,
        $version,
        $history = '',
    ...
    public static function insertXML($pid, $dsarray, $ingestObject)
    {
        ...
        if (APP_VERSION_UPLOADS_AND_LINKS == "ON" && !in_array("FezACML", $specify_list)) {
            $datastreamXMLHeaders = Misc::processLinkVersioning(
                $pid,
                $datastreamXMLHeaders,
                $datastreamXMLContent,
                $existingDatastreams
            );
        }
        if ($ingestObject) {
        ...
        foreach ($datastreamXMLHeaders as $dsKey => $dsTitle) {
            ...
            if (($dsIDName == "DC") && (!$ingestObject)) {
                ...
                } elseif ($dsTitle['CONTROL_GROUP'] == "M") {
                    $versionable = APP_VERSION_UPLOADS_AND_LINKS == "ON" ? 'true' : 'false';
            ...
            if ($xmlObj != false) {
                ...
                if (Fedora_API::datastreamExists($pid, $FezACML_dsID)) {
                    Fedora_API::callModifyDatastreamByValue(
                        ...
                        $FezACML_dsID,
                        "A",
                        "FezACML security for datastream - ".$dsID,
                        $xmlObj,
                        "text/xml",
                        "true"
                    );
    ...
    public static function generatePresmd($pid, $dsIDName)
    {
        ...
        if ($presmd_check != false) {
            ...
            Fedora_API::getUploadLocationByLocalRef(
                ...
                $presmd_id,
                $presmd_check,
                "PresMD for datastream - " . $dsIDName,
                "text/xml",
                "M"
            );
   ...
   **/
    public static function updateDatastreamLabel($pid, $dsID, $newLabel)
    {
        ...
        Fedora_API::callModifyDatastreamByReference(
            ...
            $currentDetails['MIMEType'],
            $currentDetails['versionable']
        );
    }
