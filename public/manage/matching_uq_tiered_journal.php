...
$tpl->assign("type", "matching_uq_tiered_journal");
...
if ($isAdministrator) {
    ...
    if ($action == 'edit') {
        ...
        $matches = Matching::getAllUQTieredMatches($pagerRow, $rows, $filter);
        $tpl->assign("matches", $matches['list']);
        $tpl->assign("list_info", $matches['list_info']);
        
    } else {
        $matches = Matching::getAllUQTieredMatches($pagerRow, $rows, '');
        $tpl->assign("matches", $matches['list']);
        $tpl->assign("list_info", $matches['list_info']);
    }
