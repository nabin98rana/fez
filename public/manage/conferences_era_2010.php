...
if ($isSuperAdministrator) {
    if (@$_POST["cat"] == "new") {
        $tpl->assign("result", Conference::insert());
    } elseif (@$_POST["cat"] == "update") {
        $tpl->assign("result", Conference::update());
    } elseif (@$_POST["cat"] == "delete") {
        Conference::remove();
    }
    if (@$_GET["cat"] == "edit") {
        $tpl->assign("info", Conference::getDetails($_GET["id"]));
    }
	if (@$_GET["cat"] == "search") {
		$filter = Pager::getParam('search_filter',$params);
		$tpl->assign("search_filter", $filter);
		$conference_list = Conference::getList($pagerRow, $rows, 'cnf_conference_name', $filter);
	} else {
		$conference_list = Conference::getList($pagerRow, $rows);
	}
