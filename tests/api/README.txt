...
0) Make sure your test fez system is fully operational.
You should be using a recent version of upstream fez (2014+).
Make sure your sanity checks are passing and that your system
can create background processes (the "Testing backgroundProcess"
sanity check).  If you've been running cli jobs with a different
user than your apache/nginx/php user, fez may not be able process
background jobs and you will have difficulty associating records
with eachother eg adding collections to communities.
...
3) create records/collections/communities as required conf.ini
   ...
   It is recommended you CREATE NEW COMMUNITIES and COLLECTIONS
   containing only a few test records as required by conf.ini.

   Instructions on how to set up test data is in conf-dist.ini .
   But YOU WILL NEED CREATE test communities, collections, records.

   PLEASE CONSULT conf-dist.ini .
   (or conf.ini if it still has the comments from conf-dist.ini);

4) create users and groups and assign them to roles for the above
   communities and collections as prescribed by conf-dist.ini
   ...
   restricted records in your conf.ini.
...
ISSUES
------------------------------------------------------------
1) Scenarios assume specific things about the display type (the
xdis_id in your conf.ini) you are using - for example the required
fields.
The real problem is it is hard to create test display types.  If we
could do that, we could display test display types and know exactly
what the requried fields are.

Some tests like @create-basic-case-crawl scenario tag have
explicit steps to populate what are presumed to be the required fields.
Scenarios like these would have to be modified if you have a different
set of required fields or differing xsdmf_title's / xsdmf_id's.
This is just an example.

2) Use of xsdmf_title instead of xsdmf_id.
Title's are changeable and ambiguous.
Some tests are written using these.
