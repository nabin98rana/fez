...
//	header("Pragma: no-cache");
	...
	{
		global $session;
		$sessionPath = strtolower(addTrailingSlash(backslashToSlash($session->getSessionDir())));
  ...
  function getFileContent($path)
  {
  	...
  	//return str_replace(array("\r", "\n", '"', "\t"), array("", "\\n", '\"', "\\t"), @file_get_contents($path));
         ...
         function isListingDocument($path)
         {
         	...
         	{// comma delimited
				$includeDir = trim(CONFIG_SYS_INC_DIR_PATTERN);
				$excludeDir = trim(CONFIG_SYS_EXC_DIR_PATTERN);
				$includeFile = trim(CONFIG_SYS_INC_FILE_PATTERN);
				$excludeFile = trim(CONFIG_SYS_EXC_FILE_PATTERN);      
         	...
         	{//regular expression
	          	if(is_dir($path) )
	         	{
	         		...
	         		if((!CONFIG_SYS_INC_DIR_PATTERN || preg_match(CONFIG_SYS_INC_DIR_PATTERN, $file)) && (!CONFIG_SYS_EXC_DIR_PATTERN || !preg_match(CONFIG_SYS_EXC_DIR_PATTERN, $file)))
	         		{
	         			 return true;	
	         		}else 
	         		{
	         	...
	         	{
	         		if( (!CONFIG_SYS_INC_FILE_PATTERN || preg_match(CONFIG_SYS_INC_FILE_PATTERN, $file)) && (!CONFIG_SYS_EXC_FILE_PATTERN || !preg_match(CONFIG_SYS_EXC_FILE_PATTERN, $file)) )
	         		{
	         			return true;
	         		}else 
	         		{
